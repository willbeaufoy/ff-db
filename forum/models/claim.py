...
class ClaimManager(NodeManager):
...
class Claim(Node):
    ...
    claimant = models.TextField(default='')
    ff_article = models.TextField(default='')
    ...
    def get_related_questions(self, count=10):
        ...
        return [Claim.objects.get(id=r['id']) for r in related_list]
    



class ClaimSubscription(models.Model):
...
class ClaimRevision(NodeRevision):
    class Meta:
        proxy = True
